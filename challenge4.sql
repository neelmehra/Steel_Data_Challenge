...
INSERT INTO Transaction2 (TransactionID, AccountID, TransactionDate, Amount)
...
/*2. What is the total number of accounts in the Accounts table?*/
 select COUNT(distinct(AccountID)) as 'Number of accounts' from accounts


 /*3. What is the total balance of all checking accounts?*/

 select SUM(balance) as 'Total Balance' from Accounts where AccountType = 'checking'

 /*4. What is the total balance of all accounts associated with customers who live in Los Angeles?*/
 select SUM(accounts.balance) as 'Total Balance' from Accounts join Customer on Accounts.CustomerID= Customer.CustomerID
 where City = 'Los Angeles'


 /*5. Which branch has the highest average account balance?*/
 select top 1 branches.BranchName, Avg(Accounts.Balance) as 'Avg Balance' from Accounts inner join Branches on Branches.BranchID = Accounts.AccountID
 group by branches.BranchName
 order by [Avg Balance] desc

 /*6. Which customer has the highest current balance in their accounts?*/
 
SELECT top 1 customer.CustomerID, CONCAT(customer.FirstName, ' ', customer.LastName) AS Customer_Name, SUM(accounts.balance) AS 'Total Balance'
FROM Customer
...
select Customer.CustomerID,CONCAT(customer.FirstName, ' ', customer.LastName) AS Customer_Name, COUNT(transaction2.transactionID) as 'transaction_count' from Customer
join Accounts
on Customer.CustomerID = Accounts.CustomerID join Transaction2
on accounts.accountid = Transaction2.AccountID
group by customer.CustomerID, customer.FirstName, customer.LastName
order by transaction_count desc

/*8.Which branch has the highest total balance across all of its accounts?*/
select Branches.BranchName, SUM(accounts.balance) AS 'Total Balance'
FROM Branches
...
SELECT customer.CustomerID, CONCAT(customer.FirstName, ' ', customer.LastName) AS Customer_Name, SUM(accounts.balance) AS 'Total Balance'
FROM Customer
JOIN Accounts ON customer.CustomerID = Accounts.CustomerID
GROUP BY customer.CustomerID, customer.FirstName, customer.LastName
ORDER BY 'Total Balance' DESC
...
select Branches.BranchName, COUNT(transaction2.transactionID) as 'transaction_count' from Branches
join Accounts
on Branches.BranchID = Accounts.CustomerID join Transaction2
on accounts.accountid = Transaction2.AccountID
group by Branches.BranchName
order by transaction_count desc
